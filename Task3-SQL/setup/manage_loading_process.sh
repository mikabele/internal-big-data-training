...
if [[ $INCREMENTAL -eq 1 ]]; then
  ...
  mysql -u $USER -p$USER_PASSWORD -h $HOST -P $PORT -e "CALL stg_fund_db.usp_clear_fund(FALSE);"
else
  python3 load_data.py -d ${DATA_DIR} -t ${TASK} -m ${MARKET} -f
  mysql -u $USER -p$USER_PASSWORD -h $HOST -P $PORT -e "CALL stg_fund_db.usp_clear_fund(TRUE);"
fi
